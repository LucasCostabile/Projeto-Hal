...
const createCart = async (cartData, id) => {
  try {
    ...
    if (cartExist) {
      let checkProductExist = cartExist.productsCart.find(
        (product) => String(product._id) === String(cartData._id)
      );
...
export { createCart, deleteCart, updateCart, getCartByIdUser, getAllCarts };
